...
import cn.carl.demo.map.MapFragmentActivity;
import cn.carl.demo.map.MapLocationActivity;
    ...
    public void decodeComplete(String result, int type, int quality, int requestCode) {
        ...
        if (result.equals(mResult)) {
            if (++mCount > 3) {//连续四次相同则显示结果（主要过滤脏数据，也可以根据条码类型自定义规则）
                ...
                Log.d(TAG, "[类型" + type + "/精度" + quality + "]" + result);
                Intent intent = new Intent(this, MapFragmentActivity.class);
//                Intent intent = new Intent(this, MapLocationActivity.class);
                intent.putExtra(Constants.BAR_TYPE, type);
                intent.putExtra(Constants.BAR_RESULT, mResult);
